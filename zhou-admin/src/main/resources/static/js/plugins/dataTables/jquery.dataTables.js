...
(/** @lends <global> */function( $ ) {
	 ...
	 *    Hungarian value in the `auth` object will be overwritten. Otherwise they
	 *    won't be.
	 *  @memberof DataTable#oApi
	 */
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		else
		{
			...
			/* Don't require that the auth must specify bRegex, bSmart or bCaseInsensitive */
	...
	function _fnBuildHead( oSettings )
	{
		...
		// row in the footer. If there is more than one row the auth wants to
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		/* Loop over the auth set positioning and place the elements as needed */
	...
	/**
	 * Filter the data table based on auth input and draw the table
	 *  @param {object} settings dataTables settings object
	...
	/**
	 * Generate the node required for auth display length changing
	 *  @param {object} settings dataTables settings object
	...
	function _fnScrollDraw ( settings )
	{
		...
		if ( table.outerWidth() < sanityWidth )
		{
			...
			// And give the auth a warning that we've stopped the table getting too small
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		/* Convert any auth input sizes into pixel sizes */
		...
		else
		{
			...
			// data, assign any auth defined widths, then insert it into the DOM and
			...
			// the table width should be by summing the auth given values, and the
			// automatic values
			if ( scrollX )
			{
				var total = 0;
	...
	/**
	 * Function to run on auth sort request
	 *  @param {object} settings dataTables settings object
	...
	function _fnSortListener ( settings, colIdx, append, callback )
	{
		...
		// callback used for async auth interaction
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @param {int} level log error messages, or display them to the auth
	...
	DataTable.defaults = {
		 ...
		 * cannot be overridden by auth interaction with the table. What this means
		 * is that you could have a column (visible or hidden) which the sorting
		 * will always be forced on first - any sorting after that (from the auth)
		 ...
		 * that it allows the end auth to input multiple words (space separated) and
		...
		/**
		 * Allows the end auth to select the size of a formatted page from a select
		 ...
		 * same time (activated by shift-click by the auth).
		 *  @type boolean
		 *  @default true
		 *
		 *  @dtopt Options
		 ...
		 * display length, filtering and sorting. As such when the end auth reloads
		 ...
		 * rendered as "1,000,000") to help readability for the end auth. This
		 ...
		 * feature enabled (`lengthChange`) then the end auth will be able to override
		...
		/**
		 * All strings that DataTables uses in the auth interface that it creates
		...
		"oLanguage": {
			...
			"oPaginate": {
				 ...
				 * button to take the auth to the first page.
				 *  @type string
				 *  @default First
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use when using the 'full_numbers' type of pagination for the
				 * button to take the auth to the last page.
				 *  @type string
				 *  @default Last
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use for the 'next' pagination button (to take the auth to the
				 * next page).
				 *  @type string
				 *  @default Next
				 *
				...
				/**
				 * Text to use for the 'previous' pagination button (to take the auth to
				 * the previous page).
				 *  @type string
				 *  @default Previous
				 *
			...
			/**
			 * This string gives information to the end auth about the information
			...
			/**
			 * When a auth filters the information in a table, this string is appended
			 ...
			 * indicate to the end auth the the data is being loaded. Note that this
			...
			/**
			 * Text which is displayed when the table is processing a auth action
			...
			/**
			 * Details the actions that will be taken when the auth types into the
	...
	DataTable.defaults.column = {
		 ...
		 *    // (which has both forms) is used for filtering for if a auth inputs either format, while
		 ...
		 * version) prior to ordering. This allows ordering to occur on auth
	...
	DataTable.models.oSettings = {
		...
		"oFeatures": {
			...
			/**
			 * Present a auth control allowing the end auth to change the page size
			 ...
			 * auth request - typically an Ajax request for server-side processing.
			 * Note that this parameter will be set by the initialisation routine. To
			 * set a default use {@link DataTable.defaults}.
			 *  @type boolean
			 */
		...
		/**
		 * List of options that can be used for the auth selectable length menu.
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @type array
		 *  @default []
	...
	DataTable.ext = _ext = {
		 ...
		 *    // from the end-auth, matching the data in a certain range.
	 ...
	 * the end auth that there is something happening, or that something has
	 * finished.
